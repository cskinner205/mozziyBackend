  ...
  async (req, res) => {
    try {
      ...
      req.files.forEach(async (value) => {
        ...
        const data = await Promise.all(result.map(async (value) => {
          if (value?.fileData) {
            let path = value.fileData.path ? value.fileData.path : value.fileData.Location
            try {
              const targetImage = {
                S3Object: {
                  Bucket: 'find-my-face-2',
              ...
              try {
                ...
                if (detectTargetImage.FaceDetails.length > 0) {
                  ...
                  if (FaceMatches && FaceMatches.length > 0) {
                    ...
                    await Promise.all(FaceMatches.map((match) => {
...
app.post("/api/uploadProfilePicture",
  upload.array('images'),
  async (req, res) => {
      ...
      req.files.map(async (value) => {
        ...
        try {
          ...
          if (detectSourceface.FaceDetails.length > 0) {
            ...
            // console.log('No faces were detected in the image.');
            res.status(400).send({ msg: "No faces were detected in the image. Please upload another Profile Picture", status: false, statusCode: 400 })
            return;
          }
        } catch (error) {
          console.error('Error:', error);
        ...
        const result = await collection.findOneAndUpdate({ _id: new ObjectId(req.body.userId) }, { $set: { profile_Image: uploadResult } }, { upsert: true, returnDocument: "after" })

        if (result) {
          res.send({ msg: 'Profile Updated successfully', status: true, imageData: result.profile_Image, statusCode: 200 })
        }
      })
    }
    catch (error) {
...
app.post('/api/upload', upload.array('images'), async (req, res) => {
  ...
  try {
    ...
    const result1 = Promise.all(req.files.map(async (value) => {
  ...
  try {
    ...
    if (user) {
      return res.status(400).send({
        message: "Failed! Email is already in use!", statusCode: 400
      });
    }
    else {
...
app.post("/createStripeAccount", async (req, res) => {
  ...
  try {
    ...
    let result = await collection.findOneAndUpdate({ email: result1.email }, { $set: { 'connectAccountId': account.id } })
        ...
        bcrypt.compare(req.body.password, result.password, (err, res) => {
          ...
          if (res) {
            ...
            resp.status(200).send({
              ...
              signedByGoogle: false,
              statusCode: 200,
              profile_Image: result?.profile_Image ? result?.profile_Image : null,
              userName: result.name,
              isNotifyUserEnabled: result?.isNotifyUserEnabled ? result?.isNotifyUserEnabled : null
            })
...
app.post("/api/getAllEvents", async (req, res) => {
  ...
  try {
    ...
    const favoriteEvents = await favouriteCollections.find({ user_id: new ObjectId(req.body.userId) }).toArray()
...
app.post("/api/getFeedEvents", async (req, res) => {
  let finalResult = []
  try {
    console.log("------------********getFeedEvents Api is run********-------------");
    await client.connect();
    const db = client.db('mozziy_new');
    ...
    const favoriteEvents = await favouriteCollections.find({ user_id: new ObjectId(req.body.userId) }).toArray()
    const favouriteEventsIds = favoriteEvents.map(fav => fav.event_id)
    const userResult = await userCollection.findOne({ _id: new ObjectId(req.body.userId) })
    ...
    let imagePath = userResult.profile_Image.path ? userResult.profile_Image.path:userResult.profile_Image.Location
    ...
    try {
      ...
      const data = await Promise.all(eventResult.map(async (value) => {
...
app.delete("/api/deleteEvent", async (req, res) => {
  try {
    ...
    const collection = db.collection('Event');
    console.log(req.body)
    const result = await collection.findOneAndUpdate({ _id: new ObjectId(req.body.id) }, { $set: { isDeletedByOwner: true } }, { upsert: true, returnDocument: "after" })
...
app.post('/addEventToFavorite', async (req, res) => {
  try {
    ...
    if (heart) {
      ...
      // console.log(result,"result")
      if (result.acknowledged) {
        res.status(200).send({ msg: "Event Removed from Favorites successfully", status: 200 })
      }
    }
    client.close()
...
app.post("/savePurchase", async (req, res) => {
  try {
    ...
    const res = await collection2.findOne({ _id: new ObjectId(req.body.owner) })

    if (res.connectAccountId != "") {
      const connectId = res.connectAccountId;
      console.log("connectId", connectId)
      console.log("req.body.stripePayment.paymentIntent.id", req.body.stripePayment.paymentIntent.id)
      await checkPaymentIntent(connectId, req.body.stripePayment.paymentIntent.id);
      // Select a collection
      ...
      res.status(400).send({ msg: "No connect account exists", statusCode: 400 })
    }
  } catch (err) {
...
app.post('/saveNotification', async (req, res) => {
  try {
    ...
    const result = await collection.findOneAndUpdate({ _id: new ObjectId(req.body.userId) }, { $set: { isNotifyUserEnabled: req.body.data } }, { upsert: true, returnDocument: "after" })
    console.log('this is the of save notification result', result)
    if (result) {
      res.status(200).send({ msg: 'Updated successfully', status: true, statusCode: 200, data: { isNotifyUserEnabled: result.isNotifyUserEnabled } })
    }
  } catch (error) {
    console.log(error)
    res.status(400).send({ msg: error, status: false, statusCode: 400 })
...
app.post('/getNotificationStatus', async (req, res) => {
  ...
  try {
    ...
    const db = client.db('mozziy_new');
    const collection = db.collection('User')
    const result = await collection.findOne({ _id: new ObjectId(req.body.userId) })
    console.log('result', result)
    if (result)
      res.send({ msg: 'data recieved successfully', status: true, data: { isNotifyUserEnabled: result.isNotifyUserEnabled } })
  } catch (error) {
    console.log(error)
    res.status(400).send({ msg: error, status: false, statusCode: 400 })
  }
...
app.post('/api/fetchProfileImage', async (req, res) => {
  ...
  try {
    ...
    if (result.profile_Image)
      res.status(200).send({ msg: 'Profile Image recieved successfully', status: true, statusCode: 200, data: { profile_Image: result?.profile_Image } })
    else {
      res.status(400).send({ msg: 'No Profile Image exists', status: false, statusCode: 400 })
    }
  } catch (error) {
    console.log(error)
    res.status(400).send({ msg: error, status: false, statusCode: 400 })
...
let token1 = 'ft8cAB4kSOGAPerPLOMKy0:APA91bHGTJt-h0YMluxkNTvrGci2EZJrpWlwndsitPio1t_74oGh2wgT32zAQZONSb9otoWHw1q4dvB3BjLZ5vTIRPsendp-WQGWJYzi0HktDlT1sfZLtCiZWhO8Qe16Dc717qvJCCqF'
...
app.post('/compareUploadedEventFaceWithProfilePics',
  upload.array('images'),
  async (req, res) => {
    try {
      let finalResult = []
      ...
      req.files.map(async (value) => {
        ...
        const result = await collection.find({ "profile_Image": { '$exists': 1 } }).toArray()
        // console.log("result of current focus", result)

        const imagesWithFaces = []
        try {
          const data = Promise.all(result.map(async (value) => {
            console.log("value.DEviceFcmtoken", value.DEVICEFCMTOKEN)
            console.log("typeof value.DEviceFcmtoken", typeof value.DEVICEFCMTOKEN)
            try {
              const targetImage = {
                S3Object: {
                  Bucket: 'find-my-face-2',
                ...
                if (detectTargetImage.FaceDetails.length > 0) {
                  ...
                  if (FaceMatches && FaceMatches.length > 0) {
                    ...
                    await Promise.all(FaceMatches.map((match) => {
                      const similarity = match.Similarity;
                      console.log('similarity:', similarity)
                      console.log("######## value.DEVICEFCMTOKEN", value.email)
                      console.log("######## value.DEVICEFCMTOKEN", value.DEVICEFCMTOKEN)
...
app.post('/checkConnectAccountExists', async (req, res) => {
  ...
  try {
    ...
    if (result.connectAccountId) {
      res.status(200).send({ msg: "Stripe account exists", Status: "Success", statusCode: 200 })
    }
    else {
      res.status(400).send({ msg: "Stripe account does not exist", Status: "Failed", statusCode: 400 })
    }
  }
  catch (err) {
    console.log("err=>", err);
...
const checkPaymentIntent = async (connectId, paymentintentid) => {
  ...
  try {
    ...
    const paymentIntent = await stripe.charges.list({ payment_intent: paymentintentid })

    console.log("paymentIntent", paymentIntent.data[0].balance_transaction)

    const balanceTransaction = await stripe.balanceTransactions.retrieve(paymentIntent.data[0].balance_transaction);
...
app.post("/api/getPurchases", async (req, res) => {
  try {
    ...
    const result = await collection.find({ purchaser: new ObjectId(req.body.userId) }).toArray()

    const collection2 = db.collection('Event')

    const events = []
    Promise.all(result.map(async (value) => {
...
app.post('/api/setFcmToken', async (req, res) => {
  try {
    ...
    if (result)
      res.status(200).send({ msg: "Fcm Token saved successfully", status: true, statusCode: 200 })
  }
  catch (error) {
    console.log(error)
    res.status(400).send({ msg: error, status: false, statusCode: 400 })
...
app.post('/api/deleteFCMTOKEN', async (req, res) => {
  try {
    ...
    const check1 = await collection.findOne({ _id: new ObjectId(req.body.userId) })
    console.log("^^^^^^^", check1, "&&&&&&&");
    // const result = await collection.findOneAndUpdate({ _id: new ObjectId(req.body.userId) },{ $set: { DEVICEFCMTOKEN : req.body.FCMTOKEN }},{ upsert:true, returnDocument: "after" })
    ...
    if (result.matchedCount > 0)
      res.status(200).send({ msg: "Fcm Token deleted successfully", status: true, statusCode: 200 })
    else {
      res.status(400).send({ msg: "Some Error", status: false, statusCode: 400 })
    }
  }
  catch (error) {
    console.log(error)
...
app.post('/api/deleteAccount', async (req, res) => {
  ...
  try {
    ...
    const check1 = await collection.deleteOne({ _id: new ObjectId(req.body.userId) })

    console.log(check1)

    if (check1)
      res.send({ msg: "Account Deleted Successfully", Status: 400 })
  }
